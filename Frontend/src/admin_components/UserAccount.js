  ...
  const fetchuser = async () => {
    try {
      const userResponse = await axios.get(
        "http://localhost:5000/userprofile/getuser"
      );
        ...
        userData.map(async (user) => {
          try {
            const addressResponse = await axios.get(
              `http://localhost:5000/userprofile/getaddress/${user._id}`
            );
    ...
    setFilteredUsers(
      users.filter(
        (user) =>
          ...
          (user.phonenumber &&
            user.phonenumber.toString().includes(lowercasedQuery)) || // Convert phone number to string
